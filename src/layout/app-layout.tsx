...
import { getBistros } from 'apiSdk/bistros';
...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const tenantRoles = ['business-owner', 'restaurant-manager', 'wait-staff', 'chef'];
  const isTenantUser = tenantRoles.some((role) => session?.user?.roles?.includes(role));
  const { data, error, isLoading } = useSWR<BistroInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/bistros` : null),
    () => getBistros({ tenant_id: session?.user?.tenantId }),
  );
  loading = isLoading;
  ...
  const LinkItems: Array<LinkItemProps> = [
    ...
    {
      name: 'Bistro',
      path: isTenantUser ? `/bistros/view/${data?.[0]?.id}` : '/bistros',
      entity: 'bistro',
