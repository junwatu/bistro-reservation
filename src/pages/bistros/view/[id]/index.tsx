...
import { getBistroById } from 'apiSdk/bistros';
...
function BistroViewPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<BistroInterface>(
    ...
    () =>
      getBistroById(id, {
        relations: ['user', 'order', 'reservation'],
      }),
  );
      ...
      <Box bg="white" p={4} rounded="md" shadow="md">
        <Flex justifyContent="space-between" mb={4}>
          ...
          {hasAccess('bistro', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
            <NextLink href={`/bistros/edit/${data?.id}`} passHref legacyBehavior>
              <Button
              ...
              {hasAccess('order', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Stack spacing={2} mb={8}>
                  <Flex justifyContent="space-between">
                    ...
                    <NextLink passHref href={`/orders/create?bistro_id=${data?.id}`}>
              ...
              {hasAccess('reservation', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Stack spacing={2} mb={8}>
                  <Flex justifyContent="space-between">
                    ...
                    <NextLink passHref href={`/reservations/create?bistro_id=${data?.id}`}>
                      <Button colorScheme="blue" mr="4" as="a">
                        Create
                      </Button>
                    </NextLink>
